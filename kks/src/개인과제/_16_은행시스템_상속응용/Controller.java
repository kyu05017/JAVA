...
public class Controller {
	...
	// 해당 클래스를 은행관련 컨트롤러
		...
		public String resregistration(String pw, String name, int bankcompanynum) {
			...
			Bank temp = null;
			if(bankcompanynum == 1) {
				temp = new Kokmin(bnum, pw, name, 0, 0);
			}
			else if(bankcompanynum == 2){
				temp = new Shinhan(bnum, pw, name, 0, 0);
			}
			else if(bankcompanynum == 3) {
				temp = new Hana(bnum, pw, name, 0, 0);
			}
			else {
				return null;
			}
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		public int inmoney(String bnum, int putmoney) {
			int i = 0;
			for(Bank temp2 : Main.banks) {
				// 동일한 계좌번호가 있으면 입금처리
				if(temp2 != null && temp2.getBanknum().equals(bnum)) {
					//해당 배열이 공백이 아니고 입력한 인수와 같으면
					Main.banks[i].setMymoney(temp2.getMymoney()+putmoney);
					return 0;
				}
				i++;
				return 2;
					...
					for(Bank temp2 : Main.banks) {
						if(temp2 != null && bknum2.equals(temp2.getBanknum())) {
							if(temp.getMymoney() < sendmoney) {
								...
								Main.banks[i].setMymoney(temp.getMymoney()-sendmoney);
								Main.banks[j].setMymoney(temp2.getMymoney()+sendmoney);
								return 2;
							}
		...
		public boolean loanmoney(String bnum,String pw, int loan) {
			...
			for(Bank temp : Main.banks) {
				if(temp != null && bnum.equals(temp.getBanknum()) && pw.equals(temp.getBankpw())) {
					Main.banks[i].setMymoney(temp.getMymoney()+loan);
					Main.banks[i].setLoanmoney(loan + (loan*0.1));
					return true;
				}
			}
			return false;
			...
			for(Bank temp : Main.banks) {
				if(temp != null && bnum.equals(temp.getBanknum()) && pw.equals(temp.getBankpw())) {
					if(temp.getMymoney() < temp.getLoanmoney()) {
						...
						Main.banks[i].setMymoney(temp.getMymoney()-(temp.getLoanmoney()+(temp.getLoanmoney()*0.1)));
						Main.banks[i].setLoanmoney(0);
						return true;
					}
				}
			}
